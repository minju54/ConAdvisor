...
//		File templateDir = JiveFileUtils.getDir(path);
		logger.debug("---[JiveScriptEntityBuilder]path : " + path);
		File templateDir = JiveFileUtils.getDir(path);
	...
	// 파일에서 line별로 읽어와 template 배열에 초기화
	...
	protected boolean parse(String fileName, String[] code) {
		...
		for ( ; parseState.getLineNum() < code.length; parseState.incrementLineNum()) { // rive파일의 코드 라인별로 등록
			logger.debug("Line: " + code[parseState.getLineNum()]);
			parseState.setElementState(code[parseState.getLineNum()]);
			
			if ( parseState.isIgnorable() ) { // 현재 라인이 주석/공백 등 무시해도 되는 문장인지 검사.
				continue;
			}
			//logger.debug("커맨드텍스트출력:"+parseState.getCommandText());
			...
			if ( parseState.getCommandType().equals(JiveScriptConstants.CMD_TOPIC)) { // 토픽 구분하는 기호일경우
				//	logger.debug("success");
					parseState.getEntityBuilder().getTopics().addTopic(parseState.getCommandText());
					parseState.setCurrentTopic(parseState.getCommandText());
			}
			
			else if (parseState.getCommandType().equals(JiveScriptConstants.CMD_DEFINE)) {
			...
			} else if (parseState.getCommandType().equals(JiveScriptConstants.CMD_TRIGGER)) { // 질문(+)인지 체크
				...
				parseResult = parseState.build();
			} else if (parseState.getCommandType().equals(JiveScriptConstants.CMD_REPLY)) { // 답변(-)인지 체크
